...
public class OutilReflection {
	...
	public static Object invokeMethod(Object pluginName, Plugin.Type type, Object cible, Object... args) throws InvocationTargetException, IllegalAccessException {
        ...
        for(Method method : methods) {
            for(Annotation annotation : method.getAnnotations()){
                ...
                for (Method mt : pluginInfos.getDeclaredMethods()) {
                    if(mt.invoke(annotation).equals(type)){
 
                        if(args.length > 0)
                            System.out.println("Invoke " + type + " | with : "+ Arrays.toString(args));
                        else if(cible != null)
                            System.out.println("Invoke " + type + " | on : " + pluginName + " | with :" + cible);
                        else
