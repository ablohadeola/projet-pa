...
public class OutilReflection {
                ...
                for (Method mt : pluginInfos.getDeclaredMethods()) { 
                    if(mt.invoke(annotation).equals(methodeName)){
                        if(args.length == 1)
                            ...
                            return method.invoke(pluginName, cible, args[0], args[1], args[2]);
                        else if(args.length == 4)
                            return method.invoke(pluginName, cible, args[0], args[1], args[2], args[3]);
                        else if(args.length == 5)
                            return method.invoke(pluginName, cible, args[0], args[1], args[2], args[3], args[4]);
                        else if(cible == null)
	...
	//Retourne une instance d'un plugin
		public static Object construire(Class<?> pluginClass, Object arg) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
